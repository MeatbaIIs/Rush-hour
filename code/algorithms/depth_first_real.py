...
class Depth_first():
    ...
    def possible_next_lists(self, current_list):
        ...
        for i, car in enumerate(self._cars):
            ...
            if moves and max(moves) > 0:
                distance = max(moves)
                next_list = deepcopy(current_list)
    ...
    def route_to_state(self, route):
        state = deepcopy(self._empty_state)
        for move in route:
            car = move[0]
            distance = move[1]
            state[car] = distance
        ...
        while self._stack:
            ...
            if len(state) < self._best_solution - 1:
                ...
                for new_list in self.possible_next_lists(last_list):
                    if tuple(new_list) in set(self._visited.keys()) and self._visited[tuple(new_list)] < len(state) + 1:
                        continue
                    ...
                    self._visited[tuple(new_list)] = len(state) #.append(new_list)
